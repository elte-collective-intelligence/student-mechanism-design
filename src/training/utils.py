...
def predict_reward_weights(
    ...
    return {
        name: predicted_weight[0, i] 
        for i, name in enumerate(REWARD_WEIGHT_NAMES)
    }
...
def extract_step_info(next_state: Dict, possible_agents: List[str]) -> Tuple[Dict, Dict, Dict]:
...
def create_action_mask(num_actions: int, possible_moves: List[int], dtype=torch.float32) -> torch.Tensor:
