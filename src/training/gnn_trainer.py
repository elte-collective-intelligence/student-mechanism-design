...
def train_gnn(args, agent_configs, logger_configs, visualization_configs):
    ...
    for epoch in range(args.epochs):
        ...
        logger.log(f"Chosen configuration: {num_agents} agents, {agent_money} money.", level="info")
        ...
        for episode in range(args.num_episodes):
            ...
            while not done:
                ...
                if agent_configs["agent_type"] == "gnn":
                    # Update MrX agent
                    mrX_next_graph_data = create_graph_data(next_state, "MrX", env).to(device)
                    mrX_agent.update(
                        mrX_graph_data,
                        mrX_action,
                        rewards.get("MrX", 0.0),
                    ...
                    for i in range(num_agents):
                        police_name = f"Police{i}"
                        police_graph_data_stored = create_graph_data(state, police_name, env).to(device)
                        police_next_graph_data = create_graph_data(next_state, police_name, env).to(device)
                        police_agent.update(
        ...
        logger.log(f"Epoch {epoch + 1}: Loss: {loss.item()}, Win Ratio: {win_ratio}", level="info")
