...
def train_mappo(args, agent_configs, logger_configs, visualization_configs):
    ...
    for epoch in range(args.epochs):
        ...
        logger.log(f"Configuration: {num_agents} agents, {agent_money} money.", level="info")
        ...
        for episode in range(args.num_episodes):
            ...
            while not done:
                ...
                mrx_obs = mrx_obs.detach().clone().to(dtype=torch.float32, device=device)
                
                # Get possible moves and create action mask
                possible_moves = env.get_possible_moves(0)
                mrX_action_mask = torch.zeros(max_action_dim, dtype=torch.float32, device=device)
                for move in possible_moves:
                    if move < max_action_dim:
                        mrX_action_mask[move] = 1.0
                
                mrX_action, mrX_log_prob, _ = mrX_agent.select_action(0, mrx_obs, mrX_action_mask)
                ...
                for i in range(num_agents):
                    ...
                    police_obs = state[police_name]["observation"]["Polices_pos"].sum(dim=1)
                    police_obs = police_obs.detach().clone().to(dtype=torch.float32, device=device)
                    
                    possible_moves = env.get_possible_moves(i + 1)
                    police_action_mask = torch.zeros(max_action_dim, dtype=torch.float32, device=device)
                    for move in possible_moves:
                        if move < max_action_dim:
                            police_action_mask[move] = 1.0
                    
                    police_action, police_log_prob, _ = police_agent.select_action(i, police_obs, police_action_mask)
                ...
                rewards_list = [rewards.get("MrX", 0.0)] + [rewards.get(f"Police{i}", 0.0) for i in range(num_agents)]
                dones_list = [terminations.get("MrX", False) or truncations.get("MrX", False)] + [
                    terminations.get(f"Police{i}", False) or truncations.get(f"Police{i}", False)
                    for i in range(num_agents)
                ]
                
                # Track episode metrics
                episode_mrx_reward += rewards.get("MrX", 0.0)
                episode_police_reward += sum(rewards.get(f"Police{i}", 0.0) for i in range(num_agents))
                episode_step += 1
            ...
            logger.log_scalar(f"mappo/episode_police_reward", episode_police_reward, episode)
        ...
        logger.log(f"MAPPO Epoch {epoch + 1}: Loss: {loss.item()}, Win Ratio: {win_ratio}", level="info")
        ...
        logger.log(f"MAPPO Epoch {epoch + 1} complete. Win ratio: {win_ratio:.3f}", level="info")
