...
def create_curriculum(
    num_epochs, base_graph_nodes, base_graph_edges, base_money, curriculum_range
):
    if num_epochs <= 1:
        return (
...
def modify_curriculum(
    win_ratio, node_curriculum, edge_curriculum, money_curriculum, modification_rate
):
...
def train(args, agent_configs, logger_configs, visualization_configs):
    ...
    logger.log(
        f"Starting training with variable agents and money settings.", level="debug"
    )
    ...
    if not hasattr(args, "agent_configurations") or not args.agent_configurations:
        raise ValueError(
            "args.agent_configurations must be a non-empty list of (num_agents, agent_money) tuples."
        )
    ...
    for epoch in range(args.epochs):
        ...
        num_agents, agent_money = (
            selected_config["num_police_agents"] + 1,
            selected_config["agent_money"],
        )  # Unpack the tuple
        logger.log(
            f"Choosen configuration: {num_agents} agents, {agent_money} money.",
            level="info",
        )
        ...
        police_action_size = env.action_space(
            "Police0"
        ).n  # Assuming all police have the same action space
        logger.log(
        ...
        for episode in range(args.num_episodes):
            ...
            while not done:
                ...
                logger.log(
                    f"Created graph data for MrX and Police agents.", level="debug"
                )
                ...
                logger.log(
                    f"MrX agent updated with reward: {rewards.get('MrX', 0.0)}",
                    level="debug",
                )

                # Update shared police agent
                for i in range(num_agents):
                    police_next_graph = create_graph_data(
                        next_state, f"Police{i}", env
                    ).to(device)
                    police_agent.update(
                        police_graphs[i],
                    ...
                    logger.log(
                        f"Police{i} agent updated with reward: {rewards.get(f'Police{i}', 0.0)}",
                        level="debug",
                    )
            ...
            logger.log(
                f"Epoch {epoch + 1}, Episode {episode + 1}, Total Reward: {total_reward}",
                level="debug",
            )
        ...
        for episode in range(args.num_eval_episodes):
            logger.log(
                f"Epoch {epoch + 1}, Evaluation Episode {episode + 1} started.",
                level="info",
            )
            state = env.reset(episode=episode)
            done = False
        ...
        node_curriculum, edge_curriculum, money_curriculum = modify_curriculum(
            win_ratio, node_curriculum, edge_curriculum, money_curriculum, 0.1
        )
        ...
        logger.log(
            f"Epoch {epoch + 1}: Computed target difficulty: {target_difficulty}",
            level="info",
        )

        # Train the DifficultyNet based on the difference between predicted and target difficulty
        target_tensor = (
            torch.FloatTensor([target_difficulty]).to(device).requires_grad_()
        )  # Move target to GPU
...
def train_mappo(args, agent_configs, logger_configs, visualization_configs):
    ...
    for epoch in range(args.epochs):
        ...
        logger.log(
            f"Number of police agents: {num_agents}, Agent money: {agent_money}, "
        )
        ...
        for episode in range(args.num_episodes):
            ...
            while not done:
                ...
                try:
                    ...
                    print(
                        f"MrX obs shape: {mrx_obs.shape}, expected obs_size: {feature_dim}"
                    )
                ...
                for police_idx in range(num_agents):
                    ...
                    try:
                        police_action, police_log_prob, _ = police_agent.select_action(
                            police_idx, police_obs, action_mask
                        )
                        ...
                        print(
                            f"Police obs shape: {police_obs.shape}, expected obs_size: {feature_dim}"
                        )
                        print(f"Action mask shape: {action_mask.shape}")
                        raise
                ...
                rewards = [rewards.get("MrX", 0.0)] + [
                    rewards.get(f"Police{i}", 0.0) for i in range(num_agents)
                ]
                dones = [
                    terminations.get("MrX", False) or truncation.get("MrX", False)
                ] + [
                    terminations.get(f"Police{i}", False)
                    or truncation.get(f"Police{i}", False)
                    for i in range(num_agents)
                ]
        ...
        for eval_ep in range(args.num_eval_episodes):
            logger.log(
                f"Epoch {epoch + 1}, Episode {eval_ep + 1} started.", level="info"
            )
            ...
            while not done:
                ...
                for police_idx in range(num_agents):
                    ...
                    police_action, _, _ = police_agent.select_action(
                        police_idx, police_obs, action_mask
                    )
                    actions.append(police_action)
        ...
        target_tensor = torch.tensor(
            target_difficulty, dtype=torch.float32, device=device
        )
...
def create_graph_data(state, agent_id, env):
    ...
    if mrX_pos is not None:
        ...
        logger.log(
            f"Agent {agent_id}: MrX position encoded at node {mrX_pos}.", level="debug"
        )

    # Highlight Police positions
    for i in range(env.number_of_agents - 1):
        police_pos = (
            state.get(f"Police{i}", {})
            .get("observation", None)
            .get("Polices_pos", None)
        )
...
def evaluate(args, agent_configs, logger_configs, visualization_configs):
    ...
    reward_weight_net = RewardWeightNet().to(device)
    reward_weight_net.load_state_dict(
        logger.load_model("RewardWeightNet"), strict=False
    )
    ...
    for config in args.agent_configurations:
        ...
        for name in [MrX_model_name, Police_model_name]:
            if not logger.model_exists(name):
                logger.log(
                    f"WARNING: the weights for the {name} do not exist!", level="info"
                )
        if agent_configs["agent_type"] == "gnn":
            mrX_agent = GNNAgent(
                node_feature_size=node_feature_size,
...
def evaluate_mappo(args, agent_configs, logger_configs, visualization_configs):
    ...
    reward_weight_net = RewardWeightNet().to(device)
    reward_weight_net.load_state_dict(
        logger.load_model("mappo_RewardWeightNet"), strict=False
    )
    reward_weight_net.eval()
    ...
    for config in args.agent_configurations:
        ...
        logger.log(
            f"Evaluating MAPPO with {num_agents} agents and {agent_money} money.",
            level="info",
        )
        ...
        mrx_obs = (
            state["MrX"]["observation"]["MrX_pos"]
            .detach()
            .clone()
            .to(dtype=torch.float32, device=device)
        )
        police_obs = (
            state["Police0"]["observation"]["Polices_pos"]
            .detach()
            .clone()
            .to(dtype=torch.float32, device=device)
        )
        ...
        mrX_agent = MappoAgent(
            ...
            global_obs_dim,
            action_dim,
            agent_configs["hidden_size"],
            device,
            agent_configs["gamma"],
            agent_configs["lr"],
            agent_configs["batch_size"],
            agent_configs["buffer_size"],
        ...
        for name in [MrX_model_name, Police_model_name]:
            if not logger.model_exists(name):
                logger.log(
                    f"WARNING: the weights for the {name} do not exist! ABORTING EVALUATION.",
                    level="info",
                )
                quit()
        ...
        for episode in range(args.num_eval_episodes):
            ...
            while not done:
                ...
                for i in range(num_agents):
                    police_obs = (
                        state[f"Police{i}"]["observation"]["Polices_pos"]
                        .detach()
                        .clone()
                        .to(dtype=torch.float32, device=device)
                        .sum(dim=1)
                ...
                for agent_id, action in agent_actions.items():
                    if action is not None:
                        state[agent_id]["action"] = torch.tensor(
                ...
                state = next_state
                terminations = {
                    aid: next_state[aid]["terminated"].squeeze()
                    for aid in env.possible_agents
                }
                truncations = {
                    aid: next_state[aid]["truncated"].squeeze()
                    for aid in env.possible_agents
                }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Train MrX and Police agents with dynamic difficulty prediction."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=argparse.SUPPRESS,
        help="State size for the agent",
    )
    parser.add_argument(
        "--action_size",
        type=int,
        default=argparse.SUPPRESS,
        help="Action size for the agent",
    )
    parser.add_argument(
        "--num_episodes",
        type=int,
    ...
    parser.add_argument(
        "--wandb_resume",
        action="store_true",
        help="Resume Weights & Biases run if it exists",
    )
    parser.add_argument(
        "--random_seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=bool,
        default=argparse.SUPPRESS,
        help="Set to True to evaluate the agents",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        help="Select a logger configuration!",
    )
    parser.add_argument(
        "--agent_configs",
        type=str,
        default="default",
        help="Select an agent configuration!",
    )
    parser.add_argument(
        "--vis_configs",
        type=str,
            ...
            for item in config_str.split(";"):
                if "," in item:
                    ...
                    agent_configurations.append(
                        (int(num_police_agents.strip()), float(agent_money.strip()))
                    )
                elif ":" in item:
                    ...
                    agent_configurations.append(
                        (config["num_police_agents"], config["agent_money"])
                    )
                else:
                    raise ValueError(
                        f"Invalid format for agent_configurations item: '{item}'"
                    )
