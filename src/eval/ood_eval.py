...
def create_ood_graph_configs(base_config: GraphGenerationConfig, ood_config: OODConfig) -> List[GraphGenerationConfig]:
    ...
    for nodes in range(ood_config.node_range[0], ood_config.node_range[1] + 1, 4):
        ...
        for mult in [ood_config.edge_multiplier_range[0], 1.0, ood_config.edge_multiplier_range[1]]:
            edges = max(nodes - 1, int(base_edges * mult))
            for seed in ood_config.seeds[:3]:  # Use first 3 seeds per config
                configs.append(GraphGenerationConfig(
                    num_nodes=nodes,
                    num_edges=edges,
...
def apply_edge_noise(adjacency: np.ndarray, weights: np.ndarray, noise_std: float, rng: np.random.Generator = None) -> np.ndarray:
...
def apply_edge_dropout(adjacency: np.ndarray, drop_prob: float, rng: np.random.Generator = None) -> np.ndarray:
...
def evaluate_on_graph_distribution(
    ...
    for cfg in configs:
        ...
        if ood_config.edge_noise_std > 0 or ood_config.edge_drop_prob > 0 or ood_config.reveal_skip_prob > 0:
...
def _aggregate_metrics(metrics_list: List[dict]) -> dict:
    ...
    aggregated = {}
    numeric_keys = [k for k in metrics_list[0].keys() if isinstance(metrics_list[0][k], (int, float))]
...
def generate_ood_report(results: List[OODResult]) -> str:
    ...
    for i, res in enumerate(results):
        ...
        lines.append(f"  Nodes: {summary['config']['nodes']}, Edges: {summary['config']['edges']}")
        lines.append(f"  Noise: {summary['config']['noise_std']}, Drop: {summary['config']['edge_drop']}")
