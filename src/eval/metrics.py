...
class EpisodeMetrics:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "mean_belief_ce": np.mean(self.belief_cross_entropies) if self.belief_cross_entropies else 0.0,
    ...
    def get_aggregated_metrics(self) -> AggregatedMetrics:
        ...
        if not self.episodes:
            return AggregatedMetrics(
                num_episodes=0, mrx_wins=0, police_wins=0,
                win_rate=0.5, win_rate_std=0.0,
                mean_episode_length=0.0, episode_length_std=0.0,
                mean_belief_ce=0.0, belief_ce_std=0.0,
                mean_tolls_paid=0.0, mean_budget_spent=0.0, mean_budget_efficiency=0.0,
                mean_time_to_catch=0.0, mean_survival_time=0.0,
            )
        ...
        police_win_lengths = [e.episode_length for e in self.episodes if e.winner == "Police"]
        ...
        return AggregatedMetrics(
            ...
            mean_budget_spent=float(np.mean([e.total_budget_spent for e in self.episodes])),
            mean_budget_efficiency=float(np.mean([
                e.total_budget_spent / max(e.initial_budget, 1) for e in self.episodes
            ])),
            mean_time_to_catch=float(np.mean(police_win_lengths)) if police_win_lengths else 0.0,
            mean_survival_time=float(np.mean(mrx_win_lengths)) if mrx_win_lengths else 0.0,
        )
...
def compute_win_rate(episodes: List[EpisodeMetrics], target: float = 0.5) -> Dict[str, float]:
...
def compute_time_metrics(episodes: List[EpisodeMetrics]) -> Dict[str, float]:
    ...
    return {
        ...
        "mean_time_to_catch": float(np.mean(police_win_lengths)) if police_win_lengths else 0.0,
        "std_time_to_catch": float(np.std(police_win_lengths)) if police_win_lengths else 0.0,
        "mean_survival_time": float(np.mean(mrx_win_lengths)) if mrx_win_lengths else 0.0,
...
def save_metrics_json(tracker: MetricsTracker, filepath: str, experiment_name: str = ""):
