...
class TransformerAgent(BaseAgent):
    def __init__(
        ...
        self.model = GraphTransformerModel(node_feature_size, pos_edge_dim=pos_dim).to(self.device)
    ...
    def compute_pos_enc(self, data):
        ...
        L_index, L_weight = get_laplacian(edge_index, num_nodes=num_nodes, normalization='sym')
        ...
        pos_enc = torch.from_numpy(eigvecs[:, 1:self.pos_dim+1]).float().to(self.device)
    ...
    def select_action(self, graph, action_mask):
        ...
        mask_np = action_mask.cpu().numpy() if torch.is_tensor(action_mask) else action_mask
    ...
    def update(self, graphs, actions, rewards, next_graphs, dones):
        ...
        if not isinstance(graphs, (list, tuple)): graphs = [graphs]
        if not isinstance(next_graphs, (list, tuple)): next_graphs = [next_graphs]
...
class GraphTransformerModel(nn.Module):
    def __init__(self, node_feature_size, pos_edge_dim=8, hidden_channels=64, heads=4):
        ...
        self.trans1 = TransformerConv(hidden_channels, hidden_channels, heads=heads, dropout=0.1)
        self.trans2 = TransformerConv(hidden_channels * heads, hidden_channels, heads=heads, dropout=0.1)
        self.trans3 = TransformerConv(hidden_channels * heads, hidden_channels, heads=1, concat=False, dropout=0.1)
