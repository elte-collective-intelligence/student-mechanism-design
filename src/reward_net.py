...
class RewardWeightNet(nn.Module):
    ...
    def predict_weights(self, num_agents: int, agent_money: float, 
                       graph_nodes: int, graph_edges: int, device=None) -> dict:
        ...
        inputs = torch.FloatTensor([[num_agents, agent_money, graph_nodes, graph_edges]]).to(device)
