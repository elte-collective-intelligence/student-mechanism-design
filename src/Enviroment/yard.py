...
class CustomEnvironment(BaseEnvironment):
    ...
    def calculate_rewards(self):
        ...
        police_distances = [
            self.get_distance(mrX_pos, police_pos)
        ...
        self.logger.log(
            f"MrX closest distance: {closest_distance}, average distance: {self.avg_distance}, ",
            level="debug",
        )
        ...
        self.logger.log_scalar(
            f"episode/epoch_{self.epoch}/MrX_distance_penalty", distance_penalty_mrX
        )
        self.logger.log_scalar(
            f"episode/epoch_{self.epoch}/MrX_avg_distance_penalty",
            avg_distance_penalty_mrX,
        )
        self.logger.log_scalar(
            f"episode/epoch_{self.epoch}/MrX_time_reward", time_reward_mrX
        )
        self.logger.log_scalar(
            f"episode/epoch_{self.epoch}/MrX_total_reward", mrX_reward
        )
        self.logger.log_scalar(
            f"episode/epoch_{self.epoch}/average_distance_to_MrX", self.avg_distance
        )
        police_dist_sum = 0.0
        ...
        self.logger.log_scalar(
            f"episode/epoch_{self.epoch}/average_distance_between_officers",
            police_dist_sum / (len(self.police_positions) ** 2),
        )
        ...
        for i, police in enumerate(self.agents[1:]):  # Skip MrX
            ...
            police_reward = (
                ...
                * (0.05 * self.timestep)  # Time penalty
            ...
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_distance_reward",
                distance_reward_police,
            )
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_grouping_penalty",
                grouping_penalty_police,
            )
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_position_reward",
                position_reward_police,
            )
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_time_penalty", time_penalty_police
            )
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_total_reward", police_reward
            )
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_coverage_reward", coverage_reward
            )
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_proximity_score", proximity_score
            )
            self.logger.log_scalar(
                f"episode/epoch_{self.epoch}/{police}_overlap_penalty", overlap_penalty
            )
        ...
        space = Dict(
            {
                ...
                "edge_index": Box(
                    low=0,
                    high=num_nodes,
                    shape=(2, self.board.edge_links.shape[0]),
                    dtype=np.int32,
                ),
                "edge_features": Box(
                    low=0,
                    high=ConnectedGraph.MAX_WEIGHT,
                    shape=(self.board.edges.shape[0],),
                    dtype=np.int32,
                ),
