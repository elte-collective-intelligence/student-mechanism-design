...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Train all experiments that have evaluate: False in their config."
            echo ""
            echo "Options:"
            ...
            echo ""
            echo "Examples:"
            echo "  $0                    # Train locally with Python"
            echo "  $0 --docker           # Train in Docker container"
            echo "  $0 --docker --gpu     # Train in Docker with GPU support"
            exit 0
            ;;
        *)
...
# Find all training experiments (evaluate: False or evaluate: false)
EXPERIMENTS=()
for exp_dir in "$EXPERIMENTS_DIR"/*/; do
    if [ -d "$exp_dir" ] && [ -f "${exp_dir}/config.yml" ]; then
        exp_name="$(basename "$exp_dir")"
        # Check if this is a training experiment
        if grep -qE "^evaluate:\s*(False|false)" "${exp_dir}/config.yml"; then
...
if [ ${#EXPERIMENTS[@]} -eq 0 ]; then
    echo "No training experiments found in $EXPERIMENTS_DIR"
    echo "Training experiments should have 'evaluate: False' in their config.yml"
    exit 1
fi

echo "========================================"
echo "Found ${#EXPERIMENTS[@]} training experiment(s):"
...
for exp_name in "${EXPERIMENTS[@]}"; do
    ...
    if [ "$USE_DOCKER" = true ]; then
        ...
        if docker run --rm $GPU_FLAG \
            ...
            echo "✓ Training '$exp_name' completed successfully."
        else
            echo "✗ Training '$exp_name' failed."
            FAILED+=("$exp_name")
        fi
    else
        if ./scripts/run_experiment.sh "$exp_name"; then
            echo "✓ Training '$exp_name' completed successfully."
        else
            echo "✗ Training '$exp_name' failed."
            FAILED+=("$exp_name")
